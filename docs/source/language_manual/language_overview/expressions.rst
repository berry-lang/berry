...
Using brackets to increase priority
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Relation operators are used to compare the magnitude of the operands.
...
The conditional operator (``? :``) (also known as "ternary operator") is similar to the **if else**
