...
static int m_time(bvm_t *vm)
{
    be_pushint(vm, (bint_t)time(NULL));
    be_return(vm);
}

static void time_insert(bvm_t *vm, const char *key, int value)
{
...
static int m_clock(bvm_t *vm)
{
    be_pushreal(vm, clock() / (breal_t)CLOCKS_PER_SEC);
    be_return(vm);
}
